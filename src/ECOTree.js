...
let ECONode = function (id, pid, dsc, w, h, c, bc, target, meta) {
  this.id = id;
  this.pid = pid;
  this.dsc = dsc;
  this.w = w;
      ...
      switch (this.render) {
        case 'CANVAS':
          ...
          s.push(
            '<div id="' +
              node.id +
              '" class="econode" style="{top:' +
              (node.YPosition + this.canvasoffsetTop) +
              '; left:' +
              (node.XPosition + this.canvasoffsetLeft) +
              '; width:' +
              node.w +
              '; height:' +
              node.h +
          ...
          if (this.config.selectMode != ECOTree.SL_NONE)
            s.push(
              'onclick="javascript:ECOTree._canvasNodeClickHandler(' +
                this.obj +
                ",event.target.id,'" +
                node.id +
                '\');" ',
          ...
          if (node.canCollapse) {
            s.push(
              '<a id="c' + node.id + '" href="javascript:' + this.obj + ".collapseNode('" + node.id + '\', true);" >',
            );
            s.push(
              '<img border=0 src="' +
        ...
        case 'VML':
          ...
          switch (this.config.nodeFill) {
            case ECOTree.NF_GRADIENT:
              s.push(
                '<v:fill type=gradient color2="' +
                  (node.isSelected ? this.config.nodeSelColor : color) +
                  '" color="#F5FFF5" angle=90 />',
              );
              break;
            case ECOTree.NF_FLAT:
...
ECOTree.prototype.toString = function () {
  ...
  switch (this.render) {
    ...
    case 'VML':
      s.push(
        '<v:group coordsize="10000, 10000" coordorigin="0, 0" style="position:absolute;width=10000px;height=10000px;" >',
      );
      s.push(this._drawTree());
      s.push('</v:group>');
      break;
